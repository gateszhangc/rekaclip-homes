...
const canonicalOpenRouterClaudeOpus46ModelMap: Record<string, string> = {
...
const canonicalizeOpenRouterClaudeOpus46Model = (
  model?: string
): string | undefined => {
  const normalizedModel = normalizeModelKey(model);
  if (!normalizedModel) {
...
const canonicalizeOpenRouterHomepageModel = (
  model?: string
): string | undefined =>
  canonicalizeOpenRouterOpenAIModel(model) ||
...
export function normalizeImportedAccountProviderAndModel(input: {
  ...
  const normalizedOpenRouterHomepageModel =
    canonicalizeOpenRouterHomepageModel(normalizedModelKey);
