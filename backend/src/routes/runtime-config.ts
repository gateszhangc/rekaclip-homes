...
import { z } from "zod";
import {
  getOpenClawHomepageProviderModeOverride,
  resolveOpenClawHomepageProviderMode,
  setOpenClawHomepageProviderModeOverride,
  type OpenClawHomepageProviderMode,
...
function allowRuntimeModeMutation(req: Request): boolean {
  ...
  if (!token) {
    ...
    logger.warn(
      { path: req.path },
      "BACKEND_RUNTIME_ADMIN_TOKEN unset: allowing homepage provider mode mutation (non-production only)"
    );
    return true;
  }
...
router.put("/homepage-provider-mode", (req, res) => {
  if (!allowRuntimeModeMutation(req)) {
    return res.status(401).json({
      error:
        "Unauthorized. Set BACKEND_RUNTIME_ADMIN_TOKEN and send Authorization: Bearer <token>, or run outside production with the token unset.",
    });
  }
  ...
  const normalized: OpenClawHomepageProviderMode =
    body.mode === "openrouter" ? "openrouter" : "mixed";
