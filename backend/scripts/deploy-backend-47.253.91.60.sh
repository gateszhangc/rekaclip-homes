...
# Work around occasional BuildKit snapshot corruption on some Docker hosts.
# Compose will still build, but using the classic builder path.
...
# Remove any stale container already binding the backend port.
# This avoids "port is already allocated" when legacy containers exist.
