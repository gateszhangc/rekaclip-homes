...
import { createDeployment, DEPLOYMENT_SEAT_UNAVAILABLE_ERROR_CODE, findActiveDeploymentByUserAndChannelToken, findReusableWhatsAppDeploymentBySubscriptionOrder, getDeployment, runDeployment, updateDeploymentStatus, } from "../services/deploy.js";
import { NO_AVAILABLE_NODE_ERROR_CODE, WHATSAPP_K8S_ONLY_ERROR_CODE, } from "../services/docker.js";
import { resolveDefaultOpenClawRuntimeState, resolveOpenClawRuntimeStateForDeployment } from "../services/runtime.js";
import { cancelCurrentDeploymentWhatsAppLogin, getDeploymentWhatsAppLoginSnapshot, restartDeploymentWhatsAppLogin, startOrReuseDeploymentWhatsAppLogin, } from "../services/whatsapp-login/index.js";
import { getDeploymentOpenClawDashboardSnapshot, startOrReuseDeploymentOpenClawDashboard, stopDeploymentOpenClawDashboard, } from "../services/openclaw-dashboard/index.js";
import { resolveOpenClawK8sDashboardTarget, resolveOpenClawK8sWhatsAppLoginTarget, WHATSAPP_SESSION_UNAUTHORIZED_ERROR_CODE, } from "../services/k8s.js";
import { logger, deployLogger } from "../utils/logger.js";
const router = Router();
const deployChannelSchema = z.enum(["telegram", "discord", "whatsapp"]);
const tierSchema = z.preprocess((value) => (typeof value === "string" ? value.toLowerCase() : value), z.enum(["starter", "pro"]));
...
const DEPLOYMENT_SEAT_UNAVAILABLE_MESSAGE = "Another deployment is already using your remaining active seat. Please wait for it to finish or stop it before deploying again.";
const NO_AVAILABLE_NODE_MESSAGE = "No available node right now. Please try again later.";
const WHATSAPP_K8S_ONLY_MESSAGE = "WhatsApp deployments currently require the k8s runtime.";
const GATEWAY_DASHBOARD_K8S_ONLY_MESSAGE = "Gateway Dashboard fallback currently requires the k8s runtime.";
...
function parseSubscriptionTier(raw) {
...
function parseSubscriptionOrderNo(raw) {
    if (typeof raw !== "string") {
        return undefined;
    }
...
export function inferDeploymentErrorCode(raw) {
...
export function normalizeDeployRequestPayload(body) {
    const parsed = deploySchema.parse(body);
    const inferredChannel = parsed.channel ||
...
export const isDeploymentOwnedByUser = (deployment, userId) => deployment?.user_id === userId;
const buildDeploymentResponse = async (deploymentId, fallback) => {
...
const resolveDeploymentWhatsAppLoginTarget = async ({ deploymentId, }) => {
    const runtime = await resolveOpenClawRuntimeStateForDeployment(deploymentId, null);
    ...
    const loginTarget = await resolveOpenClawK8sWhatsAppLoginTarget(deploymentId, runtime.k8sNamespace || undefined);
    return {
        error: null,
        target: {
...
const resolveDeploymentOpenClawDashboardTarget = async ({ deploymentId, }) => {
    const runtime = await resolveOpenClawRuntimeStateForDeployment(deploymentId, null);
    if (runtime.provider !== "k8s") {
        return {
            error: {
    ...
    const dashboardTarget = await resolveOpenClawK8sDashboardTarget(deploymentId, runtime.k8sNamespace || undefined);
    return {
        error: null,
        target: dashboardTarget,
    };
};
const buildDashboardProxyBody = (req) => {
...
const buildDashboardProxyHeaders = (req, localPort) => {
...
const proxyOpenClawDashboardRequest = async (req, res, deploymentId) => {
    ...
    try {
        ...
        const upstreamResponse = await fetch(`http://127.0.0.1:${snapshot.localPort}${rawPath}${search}`, {
            method: req.method,
...
router.post("/", requireAuth, async (req, res) => {
    ...
    try {
        ...
        const orderNoFromHeader = parseSubscriptionOrderNo(req.headers["x-subscription-order-no"]);
        ...
        const existingDeployment = parsed.channel === "whatsapp"
        ...
        if (existingDeployment) {
            return res.json(await buildDeploymentResponse(existingDeployment.id, {
        ...
        try {
            ...
            if (error instanceof Error &&
                ...
                const concurrentDeployment = parsed.channel === "whatsapp"
                    ? await findReusableWhatsAppDeploymentBySubscriptionOrder({
                        userId,
                        subscriptionOrderNo,
                    })
                ...
                if (concurrentDeployment) {
                    return res.json(await buildDeploymentResponse(concurrentDeployment.id, {
