...
import { default as deployRouter, inferDeploymentErrorCode, isDeploymentOwnedByUser, normalizeDeployRequestPayload, } from "./deploy.js";
...
test("normalizeDeployRequestPayload rejects unsupported channels", () => {
    assert.throws(() => normalizeDeployRequestPayload({
        channel: "slack",
...
test("inferDeploymentErrorCode recognizes the standardized Telegram and Kie deployment failures", () => {
    assert.equal(inferDeploymentErrorCode("DISCORD_MESSAGE_CONTENT_INTENT_DISABLED: Message Content Intent missing"), "DISCORD_MESSAGE_CONTENT_INTENT_DISABLED");
    assert.equal(inferDeploymentErrorCode("TELEGRAM_UNAUTHORIZED: telegram returned 401"), "TELEGRAM_UNAUTHORIZED");
    assert.equal(inferDeploymentErrorCode("WHATSAPP_SESSION_UNAUTHORIZED: whatsapp session unauthorized"), "WHATSAPP_SESSION_UNAUTHORIZED");
    assert.equal(inferDeploymentErrorCode("KIE_CREDIT_EXHAUSTED: credits=0"), "KIE_CREDIT_EXHAUSTED");
    assert.equal(inferDeploymentErrorCode("KIE_UPSTREAM_UNAVAILABLE: stage=smoke, status=429"), "KIE_UPSTREAM_UNAVAILABLE");
    assert.equal(inferDeploymentErrorCode("KIE_MODEL_PRECHECK_FAILED: empty assistant text"), "KIE_MODEL_PRECHECK_FAILED");
    assert.equal(inferDeploymentErrorCode("WHATSAPP_K8S_ONLY: runtime provider is docker"), "WHATSAPP_K8S_ONLY");
});
...
test("whatsapp second-stage and dashboard fallback routes do not require auth and remove activate", () => {
    const getRouteLayer = (path, method) => deployRouter.stack.find((layer) => layer.route?.path === path && layer.route?.methods?.[method]) || null;
    for (const [path, method] of [
        ...
        assert.equal(handlers.includes(requireAuth), false, `${method.toUpperCase()} ${path} should not require auth`);
    }
    assert.equal(getRouteLayer("/:id/whatsapp-login/activate", "post"), null, "activate route should be removed");
});
