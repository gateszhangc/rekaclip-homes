...
import { DISCORD_MESSAGE_CONTENT_INTENT_DISABLED_ERROR_CODE, KIE_CLAUDE_PROVIDER, KIE_CLAUDE_OPUS_4_6_MODEL, KIE_CODEX_PROVIDER, KIE_GEMINI_PROVIDER, KIE_GEMINI_3_1_PRO_MODEL, KIE_GPT_5_4_MODEL, KIE_PROVIDER, assertModelProviderConsistency, buildOpenClawVolumePrepScript, canBypassRuntimeCatalogLookup, getModelAuthProviderId, inferRequiredProvider, parseOpenClawChannelStatus, resolveModelFromRuntimeCatalog, resolveOpenClawModel, } from "./docker.js";
import { KIE_CLAUDE_AUTH_PROXY_BASE_URL, KIE_CLAUDE_AUTH_PROXY_PORT, buildKieClaudeAuthProxyRuntimeScript, buildKieGatewaySmokeArgs, extractOpenClawAgentTextFromJson, } from "./openclaw/kie-claude-auth-proxy.js";
...
const OPENCLAW_K8S_QUICK_EXEC_TIMEOUT_MS = Number(process.env.OPENCLAW_K8S_QUICK_EXEC_TIMEOUT_MS || 120000);
...
const OPENCLAW_KIE_PREFLIGHT_TIMEOUT_MS = Number(process.env.OPENCLAW_KIE_PREFLIGHT_TIMEOUT_MS || 20000);
const OPENCLAW_KIE_GATEWAY_SMOKE_TIMEOUT_MS = Number(process.env.OPENCLAW_KIE_GATEWAY_SMOKE_TIMEOUT_MS || 90000);
export const resolveOpenClawK8sAutoUpdateForOpenRouter = () => process.env.OPENCLAW_K8S_AUTO_UPDATE_FOR_OPENROUTER === "true";
const OPENCLAW_K8S_AUTO_UPDATE_FOR_OPENROUTER = resolveOpenClawK8sAutoUpdateForOpenRouter();
const OPENCLAW_STRICT_MODEL_MATCH = process.env.OPENCLAW_STRICT_MODEL_MATCH !== "false";
const OPENCLAW_K8S_REQUEST_CPU = process.env.OPENCLAW_K8S_REQUEST_CPU?.trim() || "500m";
const OPENCLAW_K8S_REQUEST_MEMORY = process.env.OPENCLAW_K8S_REQUEST_MEMORY?.trim() || "1Gi";
const OPENCLAW_K8S_LIMIT_CPU = process.env.OPENCLAW_K8S_LIMIT_CPU?.trim() || "2";
const OPENCLAW_K8S_LIMIT_MEMORY = process.env.OPENCLAW_K8S_LIMIT_MEMORY?.trim() || "4Gi";
const OPENCLAW_K8S_IMAGE_PULL_POLICY = process.env.OPENCLAW_K8S_IMAGE_PULL_POLICY?.trim() || "IfNotPresent";
...
const WHATSAPP_RUNTIME_READY_LOG_LINE = "Listening for personal WhatsApp inbound messages.";
...
export const WHATSAPP_SESSION_UNAUTHORIZED_ERROR_CODE = "WHATSAPP_SESSION_UNAUTHORIZED";
...
export const KIE_MODEL_PRECHECK_FAILED_ERROR_CODE = "KIE_MODEL_PRECHECK_FAILED";
...
const parseNodeSelector = () => {
    ...
    const selector = Object.fromEntries(Object.entries(parsed)
...
const describeUnknownError = (error) => {
    if (error instanceof Error) {
...
const truncateForError = (value, maxChars = 300) => value.length <= maxChars ? value : `${value.slice(0, maxChars)}...`;
const buildDeploymentError = (code, detail) => new Error(`${code}: ${detail}`);
...
export const parseKieCreditBalance = (payload) => {
    ...
    const queue = Object.values(rootPayload);
...
const parseKieBodyCode = (payload) => {
    if (!payload || typeof payload !== "object") {
        return null;
    }
    return toFiniteNumber(payload.code);
};
const assertKieBodyCodeSuccess = ({ stage, resolvedModel, payload, bodyText, }) => {
...
const extractTextFromContentValue = (value) => {
    if (typeof value === "string") {
        const normalized = value.trim();
        return normalized ? normalized : null;
    }
    ...
    const directText = extractTextFromContentValue(value.text);
    if (directText) {
        return directText;
    }
    return extractTextFromContentValue(value.content);
};
export const extractKieAssistantText = (resolvedModel, payload) => {
    if (!payload || typeof payload !== "object") {
        return null;
    }
    if (resolvedModel === KIE_GPT_5_4_MODEL) {
        const outputText = extractTextFromContentValue(payload.output_text);
        if (outputText) {
            return outputText;
        }
        return extractTextFromContentValue(payload.output);
    }
    if (resolvedModel === KIE_CLAUDE_OPUS_4_6_MODEL) {
        return extractTextFromContentValue(payload.content);
    }
    if (resolvedModel === KIE_GEMINI_3_1_PRO_MODEL) {
        const choices = payload.choices;
        if (Array.isArray(choices)) {
            for (const choice of choices) {
                const text = extractTextFromContentValue(choice.message) ||
                    extractTextFromContentValue(choice.text) ||
                    extractTextFromContentValue(choice.delta);
                if (text) {
                    return text;
                }
            }
...
export const buildKieModelPreflightRequest = (resolvedModel, apiKey) => {
    ...
    throw buildDeploymentError(KIE_MODEL_PRECHECK_FAILED_ERROR_CODE, `unsupported KIE model: ${resolvedModel || "<unset>"}`);
};
export const classifyKiePreflightHttpFailure = ({ stage, status, resolvedModel, bodyText, }) => {
    const detail = [
        `stage=${stage}`,
        `status=${status}`,
        `model=${resolvedModel}`,
...
export const performKieLivePreflight = async ({ apiKey, resolvedModel, fetchImpl = fetch, }) => {
    const runFetch = async (input, init) => {
        try {
            ...
            throw buildDeploymentError(KIE_UPSTREAM_UNAVAILABLE_ERROR_CODE, `network failure while contacting KIE: ${describeUnknownError(error)}`);
        }
    };
    ...
    if (credits === null) {
        throw buildDeploymentError(KIE_MODEL_PRECHECK_FAILED_ERROR_CODE, `stage=credit, model=${resolvedModel}, credits missing`);
    }
    if (credits <= 0) {
        throw buildDeploymentError(KIE_CREDIT_EXHAUSTED_ERROR_CODE, `stage=credit, model=${resolvedModel}, credits=${credits}`);
    }
    ...
    if (!smokePayload) {
        throw buildDeploymentError(KIE_MODEL_PRECHECK_FAILED_ERROR_CODE, `stage=smoke, model=${resolvedModel}, invalid JSON body`);
    }
    const assistantText = extractKieAssistantText(resolvedModel, smokePayload);
    if (!assistantText) {
        throw buildDeploymentError(KIE_MODEL_PRECHECK_FAILED_ERROR_CODE, `stage=smoke, model=${resolvedModel}, empty assistant text`);
    }
    return {
        credits,
        text: assistantText,
...
const extractKiePreflightCodeMessage = (value) => {
    const normalized = value.match(/(KIE_CREDIT_EXHAUSTED|KIE_UPSTREAM_UNAVAILABLE|KIE_MODEL_PRECHECK_FAILED):[^\n\r]*/);
    return normalized?.[0]?.trim() || null;
};
const buildKiePodLocalPreflightScript = (resolvedModel, mode = "full") => {
    const smokeRequest = buildKieModelPreflightRequest(resolvedModel, KIE_POD_LOCAL_API_KEY_PLACEHOLDER);
    return [
...
const parseKiePodLocalCreditCheckSuccess = (output, resolvedModel) => {
    const payload = parseJsonSafely(output);
    const credits = payload && typeof payload === "object"
        ? toFiniteNumber(payload.credits)
        : null;
    if (credits === null) {
        throw buildDeploymentError(KIE_MODEL_PRECHECK_FAILED_ERROR_CODE, `pod-local credit check returned invalid payload for model=${resolvedModel}`);
    }
    return { credits };
};
const parseKiePodLocalPreflightSuccess = (output, resolvedModel) => {
    const payload = parseJsonSafely(output);
    const credits = payload && typeof payload === "object"
        ? toFiniteNumber(payload.credits)
        : null;
    const text = payload && typeof payload === "object"
        ? extractTextFromContentValue(payload.text)
        : null;
    if (credits === null || !text) {
        throw buildDeploymentError(KIE_MODEL_PRECHECK_FAILED_ERROR_CODE, `pod-local preflight returned invalid payload for model=${resolvedModel}`);
    }
    return { credits, text };
};
const runKiePodLocalScript = async ({ resolvedModel, mode, exec, pod, runScriptInPod, retryDelaysMs, }) => {
    const script = buildKiePodLocalPreflightScript(resolvedModel, mode);
    const runner = runScriptInPod ||
        (async (nodeScript) => {
            if (!exec || !pod) {
            ...
            const result = await execInPod(exec, pod, {
                ...
                timeoutMs: OPENCLAW_KIE_PREFLIGHT_TIMEOUT_MS + 5000,
            });
            return {
    ...
    try {
        ...
        if (isRetryableK8sPodExecControlPlaneError(error)) {
            throw buildDeploymentError(KIE_UPSTREAM_UNAVAILABLE_ERROR_CODE, `pod-local preflight failed due to K8s transport error for model=${resolvedModel}: ${truncateForError(message)}`);
        }
        if (/timed out|timeout|abort/i.test(message)) {
            throw buildDeploymentError(KIE_UPSTREAM_UNAVAILABLE_ERROR_CODE, `pod-local ${mode} timed out for model=${resolvedModel}`);
        }
        throw buildDeploymentError(KIE_MODEL_PRECHECK_FAILED_ERROR_CODE, `pod-local ${mode} failed for model=${resolvedModel}: ${truncateForError(message)}`);
    }
};
export const performKiePodLocalCreditCheck = async ({ resolvedModel, exec, pod, runScriptInPod, retryDelaysMs, }) => {
...
export const performKiePodLocalPreflight = async ({ resolvedModel, exec, pod, runScriptInPod, retryDelaysMs, }) => {
    const result = await runKiePodLocalScript({
        resolvedModel,
        mode: "full",
        exec,
...
const parseKieGatewaySmokeSuccess = (output, resolvedModel) => {
    const assistantText = extractOpenClawAgentTextFromJson(output);
    if (!assistantText) {
        throw buildDeploymentError(KIE_MODEL_PRECHECK_FAILED_ERROR_CODE, `gateway smoke returned invalid payload for model=${resolvedModel}`);
    }
    if (assistantText !== "OK") {
        throw buildDeploymentError(KIE_MODEL_PRECHECK_FAILED_ERROR_CODE, `gateway smoke returned unexpected assistant text for model=${resolvedModel}: ${truncateForError(assistantText)}`);
    }
    return assistantText;
};
const performKieGatewaySmokeInPod = async ({ resolvedModel, exec, pod, }) => {
    try {
        const result = await withRetryableK8sControlPlaneCall({
            operation: `kie.gateway-smoke:${pod.namespace}/${pod.name}:${resolvedModel}`,
            isRetryable: isRetryableK8sPodExecControlPlaneError,
            fn: async () => await execInPod(exec, pod, {
                cmd: buildOpenClawK8sCliCommand(buildKieGatewaySmokeArgs(), KIE_PROVIDER),
        ...
        const message = describeUnknownError(error);
        if (isRetryableK8sPodExecControlPlaneError(error)) {
            throw buildDeploymentError(KIE_UPSTREAM_UNAVAILABLE_ERROR_CODE, `gateway smoke failed due to K8s transport error for model=${resolvedModel}: ${truncateForError(message)}`);
        }
        if (/timed out|timeout|abort|request ended without sending any chunks/i.test(message)) {
            throw buildDeploymentError(KIE_UPSTREAM_UNAVAILABLE_ERROR_CODE, `gateway smoke failed for model=${resolvedModel}: ${truncateForError(message)}`);
        }
        throw buildDeploymentError(KIE_MODEL_PRECHECK_FAILED_ERROR_CODE, `gateway smoke failed for model=${resolvedModel}: ${truncateForError(message)}`);
    }
};
const isTelegramUnauthorizedSignal = (lastError, probeStatus) => probeStatus === 401 ||
    /unauthorized|invalid token|not authorized/i.test(lastError || "");
const isWhatsAppUnauthorizedSignal = ({ lastError, probeStatus, logs, }) => probeStatus === 401 ||
...
const parseOpenClawHealth = (output) => {
    const trimmed = output.trim();
    if (!trimmed) {
...
export const isOpenClawHealthReadyForChannel = (health, channel) => {
    const channelHealth = health.channels?.[channel];
    if (!health.ok || !channelHealth?.configured || channelHealth.lastError) {
        return false;
    }
    if (channel === "telegram" &&
        isTelegramUnauthorizedSignal(channelHealth.lastError || channelHealth.probe?.error || null, channelHealth.probe?.status ?? null)) {
        return false;
    }
    if (channel === "telegram") {
...
export const isWhatsAppRuntimeReadyFromLogs = (logs) => logs.includes(WHATSAPP_RUNTIME_READY_LOG_LINE);
...
const extractExecExitCode = (status) => {
    if (!status) {
        return 0;
    }
...
export const buildOpenClawK8sCliCommand = (args, _provider) => ["env", "HOME=/home/node", "openclaw", ...args];
...
export const buildOpenClawKieConfigJson = () => `${JSON.stringify({
    models: {
        mode: "merge",
        providers: buildOpenClawKieProvidersConfig(),
    },
}, null, 2)}\n`;
export const resolveKieRuntimeModelForK8s = (requestedModel) => {
    const normalizedModel = requestedModel?.trim();
    if (!normalizedModel || !KIE_K8S_SUPPORTED_MODELS.has(normalizedModel)) {
        throw buildDeploymentError(KIE_MODEL_PRECHECK_FAILED_ERROR_CODE, `unsupported KIE model for K8s deploy: ${normalizedModel || "<unset>"}`);
    }
    return normalizedModel;
};
export const buildRenderedOpenClawConfigForK8s = ({ channel, provider, resolvedModel, }) => {
    const normalizedProvider = provider?.trim().toLowerCase() || "openai";
    if (normalizedProvider !== KIE_PROVIDER) {
        throw new Error(`buildRenderedOpenClawConfigForK8s only supports provider=${KIE_PROVIDER}`);
    }
...
export const buildOpenClawWorkspaceMemorySeedScript = (referenceDate = new Date()) => {
    const normalizedReferenceDate = new Date(referenceDate);
    const today = normalizedReferenceDate.toISOString().slice(0, 10);
    const yesterday = new Date(normalizedReferenceDate.getTime() - 24 * 60 * 60 * 1000)
        .toISOString()
        .slice(0, 10);
    return [
...
const buildOpenClawK8sGatewayWaitScript = () => [
    "set -eu",
...
export const buildOpenClawK8sConfigFingerprint = ({ channel, provider, model, }) => Buffer.from(JSON.stringify({
    version: OPENCLAW_K8S_CONFIG_FINGERPRINT_VERSION,
    gatewayMode: provider?.trim().toLowerCase() === KIE_PROVIDER ? "local" : GATEWAY_MODE,
    image: IMAGE_NAME,
    channel,
    provider: provider?.trim().toLowerCase() || null,
    model: model?.trim() || null,
    kieClaudeProxyRevision: provider?.trim().toLowerCase() === KIE_PROVIDER ? 1 : 0,
}), "utf8").toString("base64url");
const buildRenderedOpenClawConfigWriteScript = ({ renderedConfig, configFingerprint, }) => {
    ...
    return [
        ...
        `PAIRING_FILE=${shellEscape(`${OPENCLAW_HOME_DIR}/credentials/telegram-pairing.json`)}`,
...
export const buildOpenClawK8sVolumePrepScript = (configFingerprint) => {
...
export const isRetryableK8sTransportError = (error) => {
...
export const withRetryableK8sControlPlaneCall = async ({ operation, fn, retryDelaysMs = [500, 1000, 2000], isRetryable = isRetryableK8sTransportError, onRetry, }) => {
...
const isRetryableK8sPodExecControlPlaneError = (error) => {
    const message = describeUnknownError(error);
    if (!message) {
        return false;
    }
...
const decodeOpenClawK8sKubeconfigB64 = (encoded) => {
...
export const loadOpenClawKubeConfig = (kubeConfig) => {
...
export const buildOpenClawK8sLabels = (deploymentId) => ({
...
export const buildOpenClawK8sDeploymentName = (deploymentId) => `openclaw-${deploymentId}`;
export const buildOpenClawK8sHostname = (deploymentId) => {
    const normalized = deploymentId.toLowerCase().replace(/[^a-z0-9]+/g, "");
    const suffix = normalized.slice(0, 12) || "runtime";
    return `oc-${suffix}`;
};
export const buildOpenClawK8sSecretName = (deploymentId) => `openclaw-secret-${deploymentId}`;
export const buildOpenClawK8sSecretManifest = ({ deploymentId, channel, channelToken, gatewayToken, apiKey, provider, }) => {
    const stringData = {
...
export const buildOpenClawK8sDeploymentManifest = ({ deploymentId, image, secretName, provider, configFingerprint, modelEnv, }) => {
...
const upsertSecret = async (core, namespace, secret) => {
...
const upsertDeployment = async (apps, namespace, deployment) => {
    ...
    try {
        ...
        await withRetryableK8sControlPlaneCall({
            ...
            fn: () => apps.replaceNamespacedDeployment({
                name,
                namespace,
                body: deployment,
            }),
...
const getDeploymentPods = async (core, namespace, deploymentId) => {
    const podList = await core.listNamespacedPod({
        namespace,
        labelSelector: `easyclaw/deployment-id=${deploymentId}`,
    });
    return [...(podList.items || [])].sort((left, right) => String(normalizeDateString(right.metadata?.creationTimestamp) || "").localeCompare(String(normalizeDateString(left.metadata?.creationTimestamp) || "")));
};
const getMostRecentPod = async (core, namespace, deploymentId) => {
    const pods = await getDeploymentPods(core, namespace, deploymentId);
    return pods[0] || null;
};
const waitForPodRunning = async ({ core, namespace, deploymentId, timeoutMs, }) => {
    ...
    while (Date.now() < deadline) {
        ...
        const container = pod.spec?.containers?.find(({ name }) => name === "openclaw");
        ...
        if (waiting?.reason && /imagepull|backoff|crash/i.test(waiting.reason)) {
            throw new Error(`OpenClaw pod ${podName} is not healthy (${waiting.reason}: ${waiting.message || "no details"})`);
        }
        if (terminated) {
            throw new Error(`OpenClaw pod ${podName} terminated while starting (exit=${terminated.exitCode}, reason=${terminated.reason || "unknown"})`);
        }
        await sleep(1000);
    }
    throw new Error(`Timed out waiting for OpenClaw pod in deployment ${deploymentName} to start`);
};
const execInPod = async (exec, pod, input) => await new Promise(async (resolve, reject) => {
    const stdoutChunks = [];
    const stderrChunks = [];
    ...
    const timeout = setTimeout(() => {
        ...
        if (!finished) {
            finished = true;
            reject(new Error(`Pod exec timed out after ${input.timeoutMs}ms: ${input.cmd.join(" ")}`));
        }
    ...
    const finalize = () => {
        ...
        if (exitCode !== 0) {
            reject(new Error(`Pod exec failed (exit=${exitCode}): ${input.cmd.join(" ")}${`${stdoutText}${stderrText}`.trim()
    ...
    try {
        ws = await exec.exec(pod.namespace, pod.name, pod.containerName, input.cmd, stdout, stderr, null, false, (receivedStatus) => {
...
const readPodLogs = async (core, pod, tailLines) => await core.readNamespacedPodLog({
    name: pod.name,
...
const resolveRequestedRuntimeModelInPod = async (exec, pod, input) => {
    ...
    const catalogResult = await execOpenClaw(exec, pod, ["models", "list", "--all", "--json"], OPENCLAW_CLI_TIMEOUT_MS, input.provider);
    const catalogKeys = parseModelKeysFromCatalog(`${catalogResult.stdout}\n${catalogResult.stderr}`);
    return ((catalogKeys.length > 0
        ? resolveModelFromRuntimeCatalog(input.model, catalogKeys)
        : input.model) ?? undefined);
};
const assertConfiguredRuntimeModelInPod = async (exec, pod, input) => {
    if (!input.runtimeModel) {
        return undefined;
    }
    const modelsStatusCommand = await execOpenClaw(exec, pod, ["models", "status", "--json"], OPENCLAW_CLI_TIMEOUT_MS, input.provider);
    ...
    if (!configuredModel) {
        throw new Error(`OPENCLAW_MODEL_STATUS_MISSING_DEFAULT: requested=${input.runtimeModel}`);
    }
    if (configuredModel !== input.runtimeModel) {
        throw new Error(`OPENCLAW_MODEL_STATUS_MISMATCH: requested=${input.runtimeModel}, actual=${configuredModel}`);
    }
    ...
    if (expectedAuthProvider) {
        ...
        if (!hasExpectedAuthProvider) {
            throw new Error(`OPENCLAW_AUTH_PROVIDER_MISSING: provider=${expectedAuthProvider}, model=${input.runtimeModel}`);
        }
        if ((modelsStatus.auth?.missingProvidersInUse || []).includes(expectedAuthProvider)) {
            throw new Error(`OPENCLAW_AUTH_PROVIDER_UNUSABLE: provider=${expectedAuthProvider}, model=${input.runtimeModel}`);
        }
    }
    return configuredModel;
};
const isOpenClawK8sConfigFingerprintApplied = async (exec, pod, configFingerprint) => {
    const result = await execInPod(exec, pod, {
        cmd: [
            "sh",
            "-lc",
...
const injectRuntimeModelIdentityHint = async (exec, pod, resolvedModel) => {
...
const execOpenClaw = async (exec, pod, args, timeoutMs = OPENCLAW_CLI_TIMEOUT_MS, provider) => await execInPod(exec, pod, {
    cmd: buildOpenClawK8sCliCommand(args, provider),
    timeoutMs,
});
export const restartOpenClawGatewayInPod = async (exec, pod) => {
    await execInPod(exec, pod, {
        cmd: [
            "sh",
            "-lc",
...
export const configureOpenClawInPod = async (exec, pod, input) => {
    const configDir = "/home/node/.openclaw";
    const agentDir = `${configDir}/agents/main/agent`;
    const ensureConfigFileOwnership = async () => {
        await execInPod(exec, pod, {
            cmd: [
                "sh",
                "-lc",
                [
                    `chown 1000:1000 ${shellEscape(`${configDir}/openclaw.json`)} ${shellEscape(`${configDir}/openclaw.json.bak`)} ${shellEscape(`${configDir}/openclaw.json.bak.1`)} ${shellEscape(`${configDir}/openclaw.json.bak.2`)} ${shellEscape(`${configDir}/openclaw.json.bak.3`)} 2>/dev/null || true`,
                ].join("\n"),
            ],
            timeoutMs: OPENCLAW_K8S_QUICK_EXEC_TIMEOUT_MS,
        });
    ...
    const execPodCommand = async (cmd, timeoutMs) => await execInPod(exec, pod, {
        cmd,
        timeoutMs,
    });
    const execProviderOpenClaw = async (args, timeoutMs = OPENCLAW_CLI_TIMEOUT_MS) => await execOpenClaw(exec, pod, args, timeoutMs, input.provider);
    await execPodCommand(["mkdir", "-p", configDir, agentDir], OPENCLAW_K8S_QUICK_EXEC_TIMEOUT_MS);
    ...
    if (input.channel === "telegram") {
        ...
        await execPodCommand(["rm", "-f", `${configDir}/credentials/telegram-pairing.json`], OPENCLAW_K8S_QUICK_EXEC_TIMEOUT_MS);
    }
    ...
    if (input.provider?.toLowerCase() === "openrouter" &&
        ...
        try {
            await execProviderOpenClaw(["update", "--yes", "--no-restart"], OPENCLAW_UPDATE_TIMEOUT_MS);
        }
        catch (error) {
            dockerLogger.warn({ error: error instanceof Error ? error.message : String(error) }, "OpenClaw update failed inside K8s runtime (continuing without update)");
        }
    }
    ...
    if (runtimeModel) {
        try {
            ...
            dockerLogger.warn({ error: error instanceof Error ? error.message : String(error) }, "Failed to inject runtime model identity hint into K8s runtime");
        }
    }
    await execInPod(exec, pod, {
        cmd: [
            ...
            "-lc",
            [
                `printf '%s' ${shellEscape(input.configFingerprint)} > ${shellEscape(OPENCLAW_K8S_CONFIG_FINGERPRINT_FILE)}`,
...
const readOpenClawHealth = async (exec, pod) => {
    const result = await execOpenClaw(exec, pod, ["health", "--json"], OPENCLAW_CHANNEL_STATUS_TIMEOUT_MS);
    return parseOpenClawHealth(`${result.stdout}\n${result.stderr}`);
};
const readOpenClawChannelStatus = async (exec, pod, channel) => {
    const result = await execOpenClaw(exec, pod, ["channels", "status", "--json"], OPENCLAW_CHANNEL_STATUS_TIMEOUT_MS);
    return parseOpenClawChannelStatus(`${result.stdout}\n${result.stderr}`, channel);
};
export const waitForOpenClawReady = async ({ core, exec, pod, channel, deploymentId, }) => {
    ...
    while (Date.now() < deadline) {
        ...
        if (phase !== "Running") {
            ...
            if (terminated) {
                ...
                throw new Error(`OpenClaw pod exited while waiting for readiness (status=${phase || "unknown"}, exit=${terminated.exitCode}). ${recentLogs}`);
            }
            if (waiting?.reason && /backoff|crash|imagepull/i.test(waiting.reason)) {
                throw new Error(`OpenClaw pod is not healthy while waiting for readiness (${waiting.reason}: ${waiting.message || "no details"})`);
            }
            await sleep(2000);
            continue;
        }
        ...
        if (channel === "telegram" &&
            channelHealth &&
            isTelegramUnauthorizedSignal(channelHealth.lastError || channelHealth.probe?.error || null, channelHealth.probe?.status ?? null)) {
            throw buildDeploymentError(TELEGRAM_UNAUTHORIZED_ERROR_CODE, `telegram channel unauthorized during readiness check: ${channelHealth.lastError ||
        ...
        if (channel === "whatsapp" &&
            ...
            throw buildDeploymentError(WHATSAPP_SESSION_UNAUTHORIZED_ERROR_CODE, `whatsapp session unauthorized during readiness check: ${channelHealth.lastError ||
                channelHealth.probe?.error ||
                "runtime reported 401/connection failure"}`);
        }
        if (channel === "discord" &&
            lastChannelStatus?.messageContentIntent?.toLowerCase() === "disabled") {
            throw buildDeploymentError(DISCORD_MESSAGE_CONTENT_INTENT_DISABLED_ERROR_CODE, "Discord Message Content Intent is disabled. Enable it in Discord Developer Portal (Bot -> Privileged Gateway Intents) before deploying.");
        }
        if (channelReady) {
            if (readySince === null) {
                readySince = Date.now();
    ...
    if (channel === "whatsapp" &&
        ...
        throw buildDeploymentError(WHATSAPP_SESSION_UNAUTHORIZED_ERROR_CODE, `whatsapp session unauthorized while waiting for readiness: ${whatsappHealth.lastError ||
            whatsappHealth.probe?.error ||
            "runtime reported 401/connection failure"}`);
    }
    throw new Error(`OpenClaw did not become ready within ${READY_WAIT_MS}ms. Last ${channel} health: ${JSON.stringify(lastHealth)}. Last logs:\n${logs}`);
};
export const runKieLivePreflightIfNeeded = async ({ deploymentId, provider, resolvedModel, exec, pod, }) => {
    if (provider !== KIE_PROVIDER) {
        return;
    }
    const normalizedResolvedModel = resolvedModel?.trim();
    if (!normalizedResolvedModel) {
        throw buildDeploymentError(KIE_MODEL_PRECHECK_FAILED_ERROR_CODE, "resolved model missing after KIE runtime configuration");
    }
    const log = deployLogger(deploymentId);
...
export const inspectOpenClawK8sRuntime = async (deploymentId, namespace = resolveOpenClawK8sNamespace()) => {
...
export const resolveOpenClawK8sWhatsAppLoginTarget = async (deploymentId, namespace = resolveOpenClawK8sNamespace()) => {
    const { core } = createK8sClients();
    const pod = await getMostRecentPod(core, namespace, deploymentId);
    const podName = pod?.metadata?.name?.trim();
    const phase = pod?.status?.phase;
    const containerName = pod?.spec?.containers?.find(({ name }) => name === "openclaw")?.name || null;
    if (!podName || !containerName || phase !== "Running") {
        throw new Error(`OpenClaw pod is not ready for WhatsApp login (deployment=${deploymentId}, pod=${podName || "missing"}, phase=${phase || "unknown"})`);
    }
    return {
        namespace,
        pod: podName,
...
export const resolveOpenClawK8sDashboardTarget = async (deploymentId, namespace = resolveOpenClawK8sNamespace()) => {
    const { core } = createK8sClients();
    const pod = await getMostRecentPod(core, namespace, deploymentId);
    const podName = pod?.metadata?.name?.trim();
    const phase = pod?.status?.phase;
    const containerName = pod?.spec?.containers?.find(({ name }) => name === "openclaw")?.name || null;
    if (!podName || !containerName || phase !== "Running") {
        throw new Error(`OpenClaw pod is not ready for Gateway Dashboard (deployment=${deploymentId}, pod=${podName || "missing"}, phase=${phase || "unknown"})`);
    }
    return {
        namespace,
...
export const activateOpenClawWhatsAppRuntime = async ({ deploymentId, namespace, pod, container, }) => {
...
export const createOpenClawK8sRuntime = async ({ channel, channelToken, model, deploymentId, namespace, userId, tier, }) => {
    const resolvedNamespace = normalizeOptionalString(namespace) || resolveOpenClawK8sNamespace();
    const log = deployLogger(deploymentId);
    ...
    const desiredRuntimeModel = provider === KIE_PROVIDER
    ...
    const deployment = buildOpenClawK8sDeploymentManifest({
        ...
        provider,
        configFingerprint,
        modelEnv: desiredRuntimeModel && provider !== "openrouter" && provider !== "kie"
            ? desiredRuntimeModel
            : null,
    });
    log.info({
    ...
    let configuredRuntimeModel;
    if (await isOpenClawK8sConfigFingerprintApplied(exec, pod, configFingerprint)) {
