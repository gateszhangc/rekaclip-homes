...
export const KIE_CLAUDE_UPSTREAM_MESSAGES_URL = "https://api.kie.ai/claude/v1/messages";
...
const readFirstHeaderValue = (headers, name) => {
...
export const normalizeBearerToken = (rawValue) => {
    if (!rawValue) {
        return null;
    }
...
export const extractKieClaudeAuthProxyToken = (headers) => {
    const normalizedHeaders = headers;
    return (normalizeBearerToken(readFirstHeaderValue(normalizedHeaders, "x-api-key")) ||
        normalizeBearerToken(readFirstHeaderValue(normalizedHeaders, "authorization")));
};
export const buildKieClaudeProxyForwardHeaders = (headers) => {
    const normalizedHeaders = headers;
    ...
    for (const [rawName, rawValue] of Object.entries(normalizedHeaders)) {
        ...
        if (KIE_CLAUDE_PROXY_EXCLUDED_REQUEST_HEADERS.has(name) ||
            !KIE_CLAUDE_PROXY_FORWARDABLE_HEADERS.has(name)) {
            continue;
        }
        const value = Array.isArray(rawValue)
            ? rawValue.filter((entry) => typeof entry === "string").join(", ")
            : rawValue;
...
const copyUpstreamHeaders = (response, upstreamHeaders) => {
...
const parseJsonWithObjectFallback = (rawValue) => {
    const trimmed = rawValue.trim();
    if (!trimmed) {
        return null;
    }
...
export const buildKieGatewaySmokeArgs = () => [
    "agent",
    "--agent",
    "main",
    "--message",
...
export const extractOpenClawAgentTextFromJson = (rawOutput) => {
    const payload = parseJsonWithObjectFallback(rawOutput);
    if (!payload || typeof payload !== "object") {
        return null;
    }
    const queue = [];
    const rootPayloads = payload.payloads;
    ...
    while (queue.length > 0) {
        ...
        seen.add(current);
        const directText = current.text;
        if (typeof directText === "string" && directText.trim()) {
            return directText.trim();
        }
        if (Array.isArray(current.payloads)) {
            queue.push(...(current.payloads || []));
        }
...
export const createKieClaudeAuthProxyServer = ({ upstreamMessagesUrl = KIE_CLAUDE_UPSTREAM_MESSAGES_URL, fetchImpl = fetch, } = {}) => createServer(async (request, response) => {
...
export const startKieClaudeAuthProxyServer = async ({ host = KIE_CLAUDE_AUTH_PROXY_HOST, port = KIE_CLAUDE_AUTH_PROXY_PORT, upstreamMessagesUrl = KIE_CLAUDE_UPSTREAM_MESSAGES_URL, fetchImpl = fetch, } = {}) => {
...
export const buildKieClaudeAuthProxyRuntimeScript = () => [
    ...
    `const FORWARDABLE_HEADERS = new Set(${JSON.stringify(Array.from(KIE_CLAUDE_PROXY_FORWARDABLE_HEADERS))});`,
    `const EXCLUDED_REQUEST_HEADERS = new Set(${JSON.stringify(Array.from(KIE_CLAUDE_PROXY_EXCLUDED_REQUEST_HEADERS))});`,
    `const EXCLUDED_RESPONSE_HEADERS = new Set(${JSON.stringify(Array.from(KIE_CLAUDE_PROXY_EXCLUDED_RESPONSE_HEADERS))});`,
