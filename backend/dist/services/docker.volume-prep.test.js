...
import { buildOpenClawVolumePrepScript, parseOpenClawVolumePrepLogs, } from "./docker.js";
...
test("parseOpenClawVolumePrepLogs detects a written marker", () => {
    assert.equal(parseOpenClawVolumePrepLogs("OPENCLAW_IMAGE_MARKER_WRITTEN=1"), "written");
});
test("parseOpenClawVolumePrepLogs detects a missing IMAGE_BUILD_DATE fallback", () => {
    assert.equal(parseOpenClawVolumePrepLogs("OPENCLAW_IMAGE_MARKER_WRITTEN=0"), "missing");
});
