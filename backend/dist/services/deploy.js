...
import { hasDeploymentTargetHostColumn, persistDeploymentTargetHost, } from "./deployment-target-host-store.js";
import { persistDeploymentRuntimeState } from "./deployment-runtime-state-store.js";
import { createOpenClawRuntimeForDeployment, inspectOpenClawRuntimeForDeployment, resolveDefaultOpenClawRuntimeState, resolveOpenClawRuntimeStateForDeployment, } from "./runtime.js";
import { isRetryableK8sTransportError } from "./k8s.js";
import { encryptSecret, decryptSecret } from "../utils/crypto.js";
import { v4 as uuidv4 } from "uuid";
import { deployLogger } from "../utils/logger.js";
const DEPLOYMENT_ACTIVE_SEAT_UNIQUE_INDEX = "uniq_public_deployments_subscription_order_active_seat";
...
const LEGACY_DEPLOY_CHANNEL = "telegram";
...
export const WHATSAPP_CHANNEL_TOKEN_PLACEHOLDER = "__easyclaw_whatsapp_placeholder__";
export const DEPLOYMENT_SEAT_UNAVAILABLE_ERROR_CODE = "DEPLOYMENT_SEAT_UNAVAILABLE";
const DEFAULT_STALE_PROVISIONING_MS = 15 * 60 * 1000;
const DEPLOYMENT_ERROR_MESSAGE_MAX_CHARS = Number(process.env.DEPLOYMENT_ERROR_MESSAGE_MAX_CHARS || 8000);
const buildDeploymentSeatUnavailableError = () => new Error(DEPLOYMENT_SEAT_UNAVAILABLE_ERROR_CODE);
const isActiveDeploymentSeatUniqueViolation = (error) => {
    const pgError = error;
...
const getStaleProvisioningMs = () => {
    const raw = process.env.DEPLOYMENT_STALE_PROVISIONING_MS;
    if (!raw)
        return DEFAULT_STALE_PROVISIONING_MS;
...
export const isRecoverableDeploymentRuntimeError = (error) => {
    ...
    return ["unknown error", "deployment failed"].some((pattern) => normalized.includes(pattern));
};
export const shouldPromoteFailedDeploymentToRunning = ({ errorMessage, runtimeInspection, }) => Boolean(runtimeInspection?.exists &&
...
const buildUnsupportedChannelError = (channel) => new Error(`UNSUPPORTED_CHANNEL: ${channel}`);
const isDeployChannel = (value) => value === "telegram" || value === "discord" || value === "whatsapp";
const resolveStoredDeployChannel = (value) => {
    if (typeof value !== "string") {
...
export const encodeStoredChannelTokenForPersistence = ({ channel, channelToken, supportsChannelColumns, }) => {
...
export const resolveDeploymentChannelTokenForPersistence = ({ channel, channelToken, }) => {
    if (channel === "whatsapp") {
...
export const decodeStoredChannelToken = ({ storedChannel, storedToken, }) => {
...
export const assertChannelSupportedByPersistence = (channel, supportsChannelColumns) => {
...
const buildChannelAwareDeploymentSql = (supportsTargetHostColumn) => ({
...
const buildLegacyTelegramMvpDeploymentSql = (supportsTargetHostColumn) => ({
    supportsTargetHostColumn,
...
export const legacyTelegramMvpDeploymentSql = buildLegacyTelegramMvpDeploymentSql(false);
let hasChannelColumnsPromise = null;
let hasAccountIdColumnPromise = null;
const hasDeploymentChannelColumns = async () => {
    if (!hasChannelColumnsPromise) {
        hasChannelColumnsPromise = query(`select
         exists (
           select 1
         ...
         ) as channel_token_present`, [DEPLOYMENTS_SCHEMA, DEPLOYMENTS_TABLE_NAME]).then((result) => result.rows[0]?.channel_type_present === true &&
            result.rows[0]?.channel_token_present === true);
    }
    return hasChannelColumnsPromise;
};
export const hasDeploymentAccountIdColumn = async () => {
    if (!hasAccountIdColumnPromise) {
        hasAccountIdColumnPromise = query(`select exists (
         select 1
         from information_schema.columns
         where table_schema = $1
           and table_name = $2
...
export const persistDeploymentAccountId = async (deploymentId, accountPoolId) => {
    ...
    await query(`update ${DEPLOYMENTS_TABLE}
     set account_id = $2,
         updated_at = now()
     where id = $1`, [deploymentId, normalizedAccountPoolId]);
};
export const resetDeploymentAccountIdColumnSupportForTests = () => {
    hasAccountIdColumnPromise = null;
};
export const setDeploymentAccountIdColumnSupportForTests = (supported) => {
    hasAccountIdColumnPromise =
        supported === null ? null : Promise.resolve(supported);
};
const getDeploymentSql = async () => {
...
export const createDeployment = async ({ userId, channel, channelToken, requestedModel, subscriptionOrderNo, }) => {
    const deploymentId = uuidv4();
    const runtimeState = await resolveDefaultOpenClawRuntimeState();
    const targetHost = runtimeState.provider === "docker" ? runtimeState.dockerTargetHost : null;
    const deploymentSql = await getDeploymentSql();
    assertChannelSupportedByPersistence(channel, deploymentSql.supportsChannelColumns);
    const tokenForPersistence = resolveDeploymentChannelTokenForPersistence({
        channel,
        channelToken,
    });
    const encryptedToken = encryptSecret(encodeStoredChannelTokenForPersistence({
        channel,
    ...
    try {
        await query(deploymentSql.insertDeployment, deploymentSql.supportsChannelColumns
    ...
    if (!deploymentSql.supportsTargetHostColumn && targetHost) {
        try {
            ...
            await query(`delete from ${DEPLOYMENTS_TABLE} where id = $1`, [deploymentId]).catch(() => undefined);
            throw error;
        }
    }
    try {
...
export const updateDeploymentStatus = async (deploymentId, status, errorMessage) => {
    const log = deployLogger(deploymentId);
    ...
    if (status === "running") {
       ...
       set status = $2,
           ...
           updated_at = now()
       where id = $1`, [deploymentId, status, sanitizedErrorMessage]);
        return;
    }
    await query(`update ${DEPLOYMENTS_TABLE}
     set status = $2,
...
export const updateDeploymentModelResolution = async (deploymentId, requestedModel, resolvedModel) => {
    await query(`update ${DEPLOYMENTS_TABLE}
     set requested_model = $2,
         resolved_model = $3,
         updated_at = now()
     where id = $1`, [deploymentId, requestedModel?.trim() || null, resolvedModel?.trim() || null]);
};
const sanitizeDeploymentErrorMessage = (value) => {
    if (typeof value !== "string") {
        return null;
    }
    const cleaned = value
...
export const getDeployment = async (deploymentId) => {
    ...
    if (deployment) {
        if (!deploymentSql.supportsChannelColumns) {
            ...
            if (encryptedToken) {
                try {
                    ...
                    deployment.channel_type = resolveStoredDeployChannel(deployment.channel_type);
                }
            }
        }
        delete deployment.channel_token_encrypted;
        delete deployment.telegram_token_encrypted;
        const runtime = await resolveOpenClawRuntimeStateForDeployment(deploymentId, deployment.target_host);
        ...
        if (status === "running") {
            const runtimeInspection = await inspectOpenClawRuntimeForDeployment(deploymentId, deployment.target_host);
            if (!runtimeInspection.exists || !runtimeInspection.running) {
                ...
                log.warn({ runtime: runtimeInspection, provider: runtime.provider }, "Running deployment is out of sync with the runtime; marking failed");
        ...
        if (status === "failed" &&
            ...
            const runtimeInspection = await inspectOpenClawRuntimeForDeployment(deploymentId, deployment.target_host);
            if (shouldPromoteFailedDeploymentToRunning({
        ...
        if (status === "provisioning") {
            ...
            if (ageMs !== null && ageMs > staleProvisioningMs) {
                const runtimeInspection = await inspectOpenClawRuntimeForDeployment(deploymentId, deployment.target_host);
                if (runtimeInspection.exists && runtimeInspection.running) {
                    log.warn({ ageMs, staleProvisioningMs, runtime: runtimeInspection }, "Stale provisioning deployment has a running runtime; promoting to running");
                    await updateDeploymentStatus(deploymentId, "running", null);
                    deployment.status = "running";
                    deployment.error_message = null;
                }
                    ...
                    const details = !runtimeInspection.exists
                        ? `${runtime.provider} runtime not found`
                        : `${runtime.provider} runtime not running (status=${runtimeInspection.status || "unknown"}, exit=${runtimeInspection.exitCode ?? "unknown"})`;
                    const message = `Deployment stalled for >${Math.round(staleProvisioningMs / 60000)} minutes (${details}). Please deploy again.`;
                    log.warn({ ageMs, staleProvisioningMs, runtime: runtimeInspection }, "Stale provisioning deployment requested; marking failed");
                    await updateDeploymentStatus(deploymentId, "failed", message);
                    deployment.status = "failed";
                    deployment.error_message = message;
                }
...
export const findActiveDeploymentByUserAndChannelToken = async ({ userId, channel, channelToken, }) => {
    const staleProvisioningMs = getStaleProvisioningMs();
    const deploymentSql = await getDeploymentSql();
    ...
    for (const row of result.rows) {
        try {
            ...
            const { channel: storedChannel, channelToken: existingToken } = decodeStoredChannelToken({
            ...
            if (existingToken === channelToken) {
                ...
                const runtime = await resolveOpenClawRuntimeStateForDeployment(row.id, row.target_host);
                if (status === "running") {
                    const runtimeInspection = await inspectOpenClawRuntimeForDeployment(row.id, row.target_host);
                    if (!runtimeInspection.exists || !runtimeInspection.running) {
                        const details = !runtimeInspection.exists
                            ? `${runtime.provider} runtime not found`
                            : `${runtime.provider} runtime not running (status=${runtimeInspection.status || "unknown"}, exit=${runtimeInspection.exitCode ?? "unknown"})`;
                        log.warn({ userId, runtime: runtimeInspection, provider: runtime.provider }, "Found running deployment without a running runtime; marking as failed");
                        await updateDeploymentStatus(row.id, "failed", `OpenClaw ${details}. Please deploy again.`);
                        continue;
                    }
                }
                if (status === "failed") {
                ...
                if (status === "provisioning") {
                    ...
                    if (runtimeInspection.exists && !runtimeInspection.running) {
                        ...
                        log.warn({ userId, runtime: runtimeInspection, provider: runtime.provider }, "Provisioning deployment has a non-running runtime; marking as failed");
                        await updateDeploymentStatus(row.id, "failed", message);
                        continue;
                    }
                    if (ageMs !== null && ageMs > staleProvisioningMs) {
                        if (runtimeInspection.exists && runtimeInspection.running) {
                            log.warn({ userId, ageMs, staleProvisioningMs, runtime: runtimeInspection }, "Found stale provisioning deployment with running runtime; promoting to running");
                            await updateDeploymentStatus(row.id, "running", null);
                            return { id: row.id, status: "running" };
                        }
                        const details = !runtimeInspection.exists
                            ? `${runtime.provider} runtime not found`
                            : `${runtime.provider} runtime not running (status=${runtimeInspection.status || "unknown"}, exit=${runtimeInspection.exitCode ?? "unknown"})`;
                        log.warn({ userId, ageMs, staleProvisioningMs, runtime: runtimeInspection }, "Found stale provisioning deployment; marking as failed");
                        await updateDeploymentStatus(row.id, "failed", `Deployment stalled for >${Math.round(staleProvisioningMs / 60000)} minutes (${details}). Please deploy again.`);
                        continue;
                    }
                }
...
export const findReusableWhatsAppDeploymentBySubscriptionOrder = async ({ userId, subscriptionOrderNo, }) => {
    ...
    for (const row of result.rows) {
        ...
        if (status === "running") {
            ...
            if (!runtimeInspection.exists || !runtimeInspection.running) {
                const details = !runtimeInspection.exists
                    ? `${runtime.provider} runtime not found`
                    : `${runtime.provider} runtime not running (status=${runtimeInspection.status || "unknown"}, exit=${runtimeInspection.exitCode ?? "unknown"})`;
                log.warn({ userId, runtime: runtimeInspection, provider: runtime.provider }, "Found running WhatsApp deployment without a running runtime; marking as failed");
                await updateDeploymentStatus(row.id, "failed", `OpenClaw ${details}. Please deploy again.`);
                continue;
            }
            return { id: row.id, status };
        ...
        if (runtimeInspection.exists && !runtimeInspection.running) {
            const message = `OpenClaw ${runtime.provider} runtime not running (status=${runtimeInspection.status || "unknown"}, exit=${runtimeInspection.exitCode ?? "unknown"}). Please deploy again.`;
            log.warn({ userId, runtime: runtimeInspection, provider: runtime.provider }, "Provisioning WhatsApp deployment has a non-running runtime; marking as failed");
            await updateDeploymentStatus(row.id, "failed", message);
            continue;
        }
        if (ageMs !== null && ageMs > staleProvisioningMs) {
            if (runtimeInspection.exists && runtimeInspection.running) {
                log.warn({ userId, ageMs, staleProvisioningMs, runtime: runtimeInspection }, "Found stale provisioning WhatsApp deployment with running runtime; reusing it");
                return { id: row.id, status };
            }
            const details = !runtimeInspection.exists
                ? `${runtime.provider} runtime not found`
                : `${runtime.provider} runtime not running (status=${runtimeInspection.status || "unknown"}, exit=${runtimeInspection.exitCode ?? "unknown"})`;
            log.warn({ userId, ageMs, staleProvisioningMs, runtime: runtimeInspection }, "Found stale provisioning WhatsApp deployment; marking as failed");
            await updateDeploymentStatus(row.id, "failed", `Deployment stalled for >${Math.round(staleProvisioningMs / 60000)} minutes (${details}). Please deploy again.`);
            continue;
        }
        return { id: row.id, status };
...
export const runDeployment = async (deploymentId, model, tier) => {
    const log = deployLogger(deploymentId);
    const deploymentSql = await getDeploymentSql();
    ...
    const runtime = await resolveOpenClawRuntimeStateForDeployment(deploymentId, row.target_host);
    log.info({
        userId,
        channel,
    ...
    try {
        ...
        await updateDeploymentModelResolution(deploymentId, container.requestedModel || model || null, container.resolvedModel || null);
        ...
        if (isRecoverableDeploymentRuntimeError(errorMessage)) {
            try {
                const runtimeInspection = await inspectOpenClawRuntimeForDeployment(deploymentId, row.target_host);
                if (shouldPromoteFailedDeploymentToRunning({
                    errorMessage,
                    runtimeInspection,
                })) {
                    log.warn({ runtime: runtimeInspection, error: errorMessage }, "Deployment hit a recoverable control-plane error after the runtime came up; promoting to running");
                    await updateDeploymentStatus(deploymentId, "running", null);
                    return;
                }
            }
            catch (runtimeError) {
                log.warn({ error: describeUnknownError(runtimeError) }, "Failed to inspect runtime while recovering deployment failure");
            }
        }
