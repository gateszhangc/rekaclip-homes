...
import { buildSshProbeCommand, buildSshProbeArgs, buildSshDockerTunnelArgs, canBypassRuntimeCatalogLookup, isLocalDockerTargetHost, parseOpenClawTargetNodeProbeOutput, resolveFixedOpenClawTargetHost, resolveOpenClawDeployMode, resolveOpenClawTargetHost, resolveOpenClawTargetHostPool, selectBestOpenClawTargetNode, } from "./docker.js";
...
test("resolveFixedOpenClawTargetHost requires an explicit target host", () => {
    withEnv({
        ...
        assert.throws(() => resolveFixedOpenClawTargetHost(), /OPENCLAW_TARGET_HOST is required/i);
    });
});
...
test("resolveOpenClawTargetHostPool accepts JSON strings and host objects", () => {
    withEnv({
        OPENCLAW_TARGET_HOST: undefined,
        OPENCLAW_TARGET_HOST_POOL_JSON: '["89.167.51.48", {"host":"89.167.51.49"}, {"host":" 89.167.51.48 "}]',
    }, () => {
        assert.deepEqual(resolveOpenClawTargetHostPool(), [
            { host: "89.167.51.48" },
            { host: "89.167.51.49" },
...
test("parseOpenClawTargetNodeProbeOutput reads cpu and memory usage", () => {
    assert.deepEqual(parseOpenClawTargetNodeProbeOutput("cpu=12.3 mem=48.7 image=1\n", "89.167.51.48"), {
        host: "89.167.51.48",
        cpuPercent: 12.3,
        memoryPercent: 48.7,
        hasImage: true,
...
test("parseOpenClawTargetNodeProbeOutput rejects malformed output", () => {
    assert.throws(() => parseOpenClawTargetNodeProbeOutput("oops", "89.167.51.48"), /Invalid probe output/i);
});
test("selectBestOpenClawTargetNode keeps only nodes below both thresholds and picks the lowest max load", () => {
    assert.deepEqual(selectBestOpenClawTargetNode([
...
test("selectBestOpenClawTargetNode returns null when every node is above threshold", () => {
    assert.equal(selectBestOpenClawTargetNode([
