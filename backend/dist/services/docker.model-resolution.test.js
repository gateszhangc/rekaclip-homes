...
import { assertModelProviderConsistency, buildProviderBootstrapConfigCommands, getModelAuthProviderId, inferRequiredProvider, resolveModelFromRuntimeCatalog, resolveOpenClawModel, setOpenClawHomepageProviderModeOverride, } from "./docker.js";
const withHomepageProviderMode = async (mode, fn) => {
...
test("homepage GPT-5.4 aliases map to OpenRouter when mode is openrouter", () => {
    return withHomepageProviderMode("openrouter", () => {
        assert.equal(resolveOpenClawModel("gpt-5-4", "openrouter"), "openrouter/openai/gpt-5.4");
...
test("legacy kie-codex GPT aliases still resolve to the canonical Kie GPT model", () => {
    assert.equal(resolveOpenClawModel("kie-codex/gpt-5-4", "kie"), "kie-gpt/gpt-5-4");
});
...
test("homepage Gemini 3 Pro aliases default to Kie when env is unset (mixed)", () => {
    return withHomepageProviderMode(undefined, () => {
        assert.equal(resolveOpenClawModel("gemini-3-pro", "kie"), "kie-gemini/gemini-3.1-pro");
...
test("explicit OpenRouter model IDs are preserved when OpenRouter is requested", () => {
    assert.equal(resolveOpenClawModel("openai/gpt-5.4", "openrouter"), "openrouter/openai/gpt-5.4");
    assert.equal(resolveOpenClawModel("google/gemini-3-pro-preview", "openrouter"), "openrouter/google/gemini-3.1-pro-preview");
    assert.equal(inferRequiredProvider("google/gemini-3-pro-preview"), "openrouter");
});
test("homepage Claude Opus 4.6 aliases default to Kie when env is unset (mixed)", () => {
    return withHomepageProviderMode(undefined, () => {
        assert.equal(resolveOpenClawModel("claude-opus-4-6", "kie"), "kie-claude/claude-opus-4-6");
        assert.equal(resolveOpenClawModel("claude-opus-4.6", "kie"), "kie-claude/claude-opus-4-6");
...
test("homepage aliases switch Claude and Gemini to OpenRouter when mode is openrouter", () => {
    return withHomepageProviderMode("openrouter", () => {
        assert.equal(resolveOpenClawModel("gpt-5-4", "openrouter"), "openrouter/openai/gpt-5.4");
        assert.equal(resolveOpenClawModel("claude-opus-4-6", "openrouter"), "openrouter/anthropic/claude-opus-4");
        assert.equal(resolveOpenClawModel("gemini-3-pro", "openrouter"), "openrouter/google/gemini-3.1-pro-preview");
        assert.equal(inferRequiredProvider("gpt-5-4"), "openrouter");
...
test("explicit provider-qualified models stay stable across homepage provider modes", () => {
    return withHomepageProviderMode("openrouter", () => {
        assert.equal(resolveOpenClawModel("kie-claude/claude-opus-4-6", "kie"), "kie-claude/claude-opus-4-6");
        assert.equal(resolveOpenClawModel("openrouter/anthropic/claude-opus-4-6", "openrouter"), "openrouter/anthropic/claude-opus-4");
        assert.equal(resolveOpenClawModel("openrouter/google/gemini-3.1-pro-preview", "openrouter"), "openrouter/google/gemini-3.1-pro-preview");
        assert.equal(inferRequiredProvider("kie-claude/claude-opus-4-6"), "kie");
        assert.equal(inferRequiredProvider("openrouter/anthropic/claude-opus-4-6"), "openrouter");
    });
});
test("explicit Kie homepage models reject non-kie providers before container creation", () => {
    assert.throws(() => assertModelProviderConsistency(resolveOpenClawModel("kie-gpt/gpt-5-4", "kie"), "openrouter"), /MODEL_PROVIDER_MISMATCH/);
});
test("custom Kie providers are bootstrapped through OpenClaw config", () => {
    ...
    const providers = JSON.parse(commands[1]?.[4] || "{}");
...
test("runtime auth validation targets Kie and OpenRouter providers only when needed", () => {
    ...
    assert.equal(getModelAuthProviderId("openrouter/openai/gpt-5.4"), "openrouter");
