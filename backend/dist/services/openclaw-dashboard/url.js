...
export function parseDashboardUrlFromOutput(output) {
    const match = output.match(DASHBOARD_URL_PATTERN);
    return match?.[1] ?? null;
}
export function rewriteDashboardUrlPort(dashboardUrl, localPort) {
    const parsed = new URL(dashboardUrl);
    parsed.hostname = "127.0.0.1";
    parsed.port = String(localPort);
    return parsed.toString();
}
export function maskDashboardUrl(dashboardUrl) {
    const parsed = new URL(dashboardUrl);
