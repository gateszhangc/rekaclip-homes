...
import { getPersistedDeploymentRuntimeState, persistDeploymentRuntimeState, readDeploymentRuntimeState, removePersistedDeploymentRuntimeState, resetDeploymentRuntimeStateStoreForTests, resolveDeploymentRuntimeStateFile, } from "./deployment-runtime-state-store.js";
...
test("resolveDeploymentRuntimeStateFile defaults under a local state directory", () => {
    ...
    delete process.env.OPENCLAW_RUNTIME_STATE_FILE;
    try {
        assert.match(resolveDeploymentRuntimeStateFile(), /state[\/\\]deployment-runtime-state\.json$/);
    }
    finally {
        if (previousStateFile === undefined) {
            delete process.env.OPENCLAW_RUNTIME_STATE_FILE;
