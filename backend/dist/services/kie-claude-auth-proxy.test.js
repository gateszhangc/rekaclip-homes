...
import { buildKieGatewaySmokeArgs, buildKieClaudeProxyForwardHeaders, extractOpenClawAgentTextFromJson, startKieClaudeAuthProxyServer, } from "./openclaw/kie-claude-auth-proxy.js";
...
test("gateway smoke helpers keep the exact OpenClaw command contract and parse payload text", () => {
    ...
    assert.equal(extractOpenClawAgentTextFromJson(JSON.stringify({
        payloads: [{ text: "OK" }],
    })), "OK");
});
test("KIE Claude auth proxy exposes /healthz and forwards /v1/messages with rewritten auth", async () => {
    ...
    try {
        ...
        assert.equal(await response.text(), '{"content":[{"type":"text","text":"OK"}]}');
        if (!upstreamHeaders) {
            assert.fail("proxy did not forward the request upstream");
        }
        const receivedHeaders = upstreamHeaders;
        ...
        assert.equal(receivedHeaders["anthropic-beta"], "tools-2026-01-01");
