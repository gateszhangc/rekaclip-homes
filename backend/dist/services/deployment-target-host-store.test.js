...
import { getPersistedDeploymentTargetHost, persistDeploymentTargetHost, readDeploymentTargetHostState, resetDeploymentTargetHostStoreForTests, resolveDeploymentTargetHostStateFile, resolvePersistedDeploymentTargetHost, setDeploymentTargetHostColumnSupportForTests, } from "./deployment-target-host-store.js";
const withTempStateFile = async (fn) => {
...
test("resolveDeploymentTargetHostStateFile defaults under a local state directory", () => {
    ...
    delete process.env.OPENCLAW_TARGET_HOST_STATE_FILE;
    try {
        assert.match(resolveDeploymentTargetHostStateFile(), /state[\/\\]deployment-target-hosts\.json$/);
    }
    finally {
        if (previousStateFile === undefined) {
            delete process.env.OPENCLAW_TARGET_HOST_STATE_FILE;
...
test("resolvePersistedDeploymentTargetHost prefers the database value when present", async () => {
    await withTempStateFile(async () => {
        ...
        await persistDeploymentTargetHost("dep-1", "89.167.51.48");
        assert.equal(await resolvePersistedDeploymentTargetHost("dep-1", " 144.91.114.233 "), "144.91.114.233");
    });
});
test("resolvePersistedDeploymentTargetHost falls back to the JSON store when the database lacks target_host", async () => {
    await withTempStateFile(async () => {
        setDeploymentTargetHostColumnSupportForTests(false);
        await persistDeploymentTargetHost("dep-1", "89.167.51.48");
        assert.equal(await resolvePersistedDeploymentTargetHost("dep-1", null), "89.167.51.48");
    });
});
