...
test("maskDashboardUrl hides the token in the hash fragment", () => {
    assert.equal(maskDashboardUrl("http://127.0.0.1:39123/control-ui/#token=codexwa-86add9d981"), "http://127.0.0.1:39123/control-ui/#token=code...d981");
});
test("CurrentDashboardSessionStore starts once and reuses the ready snapshot", async () => {
    ...
    const store = new CurrentDashboardSessionStore({
        runtimeAdapter: {
            async launch({ target: runtimeTarget, onLog }) {
                ...
                return {
                    localPort: 39123,
                    dashboardUrl: "http://127.0.0.1:39123/control-ui/#token=codexwa-86add9d981",
    ...
    assert.equal(first.localPort, 39123);
    assert.equal(first.maskedDashboardUrl, "http://127.0.0.1:39123/control-ui/#token=code...d981");
