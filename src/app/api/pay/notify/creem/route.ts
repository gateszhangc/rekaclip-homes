...
const log = createLogger("api/pay/notify/creem");
...
export async function POST(req: Request) {
  try {
    ...
    log.info({ req }, "接受到creem的webhook的通知");    
    ...
    switch (event.eventType) {
      case "checkout.completed": {
        ...
        log.info({ order_no, paid_email, paid_detail }, "处理creem的webhook的checkout.completed事件通知");           
        break;
      }
    ...
    log.info({ e }, "creem notify failed:");            
